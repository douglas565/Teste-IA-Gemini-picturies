...
const ResultCard: React.FC<ResultCardProps> = ({ item, onEdit, onImageClick }) => {
  ...
  return (
    <div className={`relative bg-white rounded-xl shadow-sm border transition-all duration-200 hover:shadow-md ${isPending ? 'border-yellow-400 ring-1 ring-yellow-100' : 'border-gray-200'}`}>
      ...
      <div className="flex p-4 gap-4">
        {/* Image Thumbnail */}
        <div 
          className="w-24 h-24 flex-shrink-0 bg-gray-100 rounded-lg overflow-hidden border border-gray-100 relative group cursor-pointer"
          onClick={() => onImageClick(item.imageUrl)}
          title="Clique para ampliar"
        >
          <img src={item.imageUrl} alt="Luminaire" className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" />
          ...
          {item.features && (
            <div className="absolute bottom-0 left-0 right-0 bg-black/60 p-0.5 text-[8px] text-white text-center opacity-0 group-hover:opacity-100 transition-opacity">
               AR: {item.features.aspectRatio.toFixed(2)}
            </div>
          )}
        </div>
